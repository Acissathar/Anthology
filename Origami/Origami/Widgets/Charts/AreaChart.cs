...
/// <summary>
/// Cartesian area chart. Plots one or more <see cref="CartesianSeries{T}"/> as a stroked polyline
/// whose region down to the zero baseline is always filled. Can be smoothed.
/// </summary>
public sealed class AreaChart<T> : CartesianCore<AreaChart<T>, T>
{
    internal AreaChart(Paper paper, string id, OrigamiTheme theme, IReadOnlyList<T>? data)
    ...
    /// <summary>Draw the series through a smoothed curve instead of straight segments.</summary>
    public AreaChart<T> Smooth() => Interpolation(CartesianInterpolation.Smooth);

    /// <summary>How consecutive points are joined: straight segments or a smoothed curve.</summary>
    public AreaChart<T> Interpolation(CartesianInterpolation interpolation) { _interpolation = interpolation; return this; }

    /// <summary>Alpha multiplier, 0 to 1, applied to the filled area beneath each series.</summary>
    public AreaChart<T> Opacity(float opacity) { _opacity = Math.Clamp(opacity, 0f, 1f); return this; }
    ...
    protected override void PaintMarks(Canvas canvas, in PlotContext ctx)
    {
        foreach (var s in ctx.Series)
        {
            ...
            PaintFill(canvas, in ctx, s, pts, strokeColor, _opacity);
            PaintStroke(canvas, pts, strokeColor, strokeWidth, s.Dash);
        }
    }


    /// <summary>Crosshair through the sampled x with a dot where every visible series' upper edge
    /// crosses it, and a readout of each series' value there.</summary>
    ...
    private static void PaintFill(Canvas canvas, in PlotContext ctx, CartesianSeries<T> s, List<Float2> pts, Color strokeColor, float opacity)
    {
        float baseline = Math.Clamp(ctx.YPos(0d), ctx.PlotT, ctx.PlotB);
        Color32 fillCol = ToC32(s.Color ?? strokeColor, opacity);
